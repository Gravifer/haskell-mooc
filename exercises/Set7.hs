...
velocity (Distance d) (Time t) = case t of
  0 -> error "Infinite velocity (division by zero)"
  _ -> Velocity (d / t)
...
add x xset@(Set xs) -- = Set . sort . nub $ (x:xs)
  | member x xset = xset
...
-- data State = Start | Error | Finished | WIP Event -- ! only encode the furthermost event, which apparently the test doesn't allow
--   deriving (Eq,Show)

-- step :: State -> Event -> State
-- step state event = case (state, event) of
--   (Start, AddEggs) -> WIP AddEggs
--   (WIP AddEggs, AddFlour) -> WIP AddFlour
--   (WIP AddFlour, AddSugar) -> WIP AddSugar
--   (WIP AddSugar, Mix) -> WIP Mix
--   (WIP Mix, Bake) -> Finished
--   (Finished, _) -> Finished -- once finished, stay finished
--   (_, _) -> Error -- any other case results in an error
...
average (x1 :| [] ) = x1
average (x1 :|rest) = sum (x1:rest) / fromIntegral (1 + length rest)
...
reverseNonEmpty ne1@(x1 :| []) = ne1
reverseNonEmpty (x1 :| xs) = last xs :| reverse (x1 : init xs)
