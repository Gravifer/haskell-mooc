...
outputInInput input output = forAll (elements output) (prop input)
  where prop :: (Eq a) => [a] -> (a, Int) -> Property
        prop xs (x, n) = length (filter (== x) xs) === n
...
frequenciesProp freq (NonEmpty input) =
  conjoin [f input output | f <- [sumIsLength, inputInOutput, outputInInput]]
