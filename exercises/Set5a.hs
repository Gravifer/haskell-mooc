...
totalPrice shoppingEntry@(MkShoppingEntry _ price count) =
...
buyOneMore shopEntry@(MkShoppingEntry name price count) =
...
getX :: Position -> Int
getX p@(Position x _)= x

-- getY returns the y of a position
getY :: Position -> Int
getY p@(Position _ y)= y
...
right p@(Position x y)= Position (x+1) y
...
fromNat n = go n 0
  where
    go Zero acc = acc
    go (PlusOne rest) acc = go rest (acc + 1)
...
-- prettyPrint bin = show (fromBin bin) -- jk
