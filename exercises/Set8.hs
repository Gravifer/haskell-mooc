...
combine c2func (Picture f1) (Picture f2) = Picture f
  where f coord = c2func (f1 coord) (f2 coord)
...
cut (Shape f1) (Shape f2) = Shape f
  where f coord = f1 coord && not (f2 coord)
------------------------------------------------------------------------------
...
paintSolid color shape@(Shape sf) base@(Picture pf) = Picture g
  where g coord | sf coord = color
                | otherwise = pf coord
------------------------------------------------------------------------------
...
-- paint pat@(Picture patf) shape@(Shape sf) base@(Picture pf) = Picture $ \coord -> bool pf patf (sf coord) coord
paint pat@(Picture patf) shape@(Shape sf) base@(Picture pf) = Picture g
  where g coord | sf coord = patf coord
                | otherwise = pf coord
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill c) _ = Picture (const c)
...
instance Transform Flip where
  apply flip_t (Picture f) = Picture $ f . case flip_t of
    FlipX  -> flipCoordX
    FlipY  -> flipCoordY
    FlipXY -> flipCoordXY
    where
...
instance Transform Blur where
  -- apply _ pic@(Picture f) = Picture g
  --   where g pxl@(Coord x y) = sum (map f (pxl : neighbours pxl)) `div` 5
  --         neighbours (Coord x y) = [Coord (x-1) y, Coord (x+1) y,
...
instance Transform BlurMany where
  apply (BlurMany n) p = iterate (apply Blur) p !! n
------------------------------------------------------------------------------
